...
std::ostream &operator<<(std::ostream &os, const ConnectionInfo &dt) {
  os << dt.src_id << ":" << dt.src_key << " <---> " << dt.dst_id << ":"
     << dt.dst_key;
  return os;
}
...
void GrcParser::parse() {
  ...
  BOOST_FOREACH (boost::property_tree::ptree::value_type const &f, flow_graph) {
    if (f.first == "block") {
      ...
      } else if (block_info.key.find("variable") ==
                 0) { // any block which starts with "variable" .. this e.g.
                      // includes all taps
...
struct NullSinkMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
                            const std::vector<BlockInfo> &variables) override {
...
struct VectorToStreamMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto vlen = info.eval_param_value<int>("vlen", variables);
    return gr::blocks::vector_to_stream::make(vlen * getSizeOfType(type),
                                              num_items);
  }
};
...
struct VectorToStreamsMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto vlen = info.eval_param_value<int>("vlen", variables);
    return gr::blocks::vector_to_streams::make(vlen * getSizeOfType(type),
                                               num_streams);
  }
};
...
struct StreamToVectorMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto num_items = info.eval_param_value<int>("num_items", variables);
    auto vlen = info.eval_param_value<int>("vlen", variables);
    return gr::blocks::stream_to_vector::make(vlen * getSizeOfType(type),
                                              num_items);
  }
};
...
struct SigSourceMaker : BlockMaker {
  ...
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    return gr::analog::sig_source_f::make(sampling_freq, waveform_type,
                                          wave_freq, ampl, offset);
  }
};
...
struct ThrottleMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto type = info.param_value<>("type");
    auto samples_per_sec =
        info.eval_param_value<double>("samples_per_second", variables);
    auto ignore_tags = info.param_value<bool>("ignoretag");
    return gr::blocks::throttle::make(getSizeOfType(type), samples_per_sec,
                                      ignore_tags);
  }
};
...
struct TagShareMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto vlen = info.eval_param_value<int>("vlen", variables);
    return gr::blocks::tag_share::make(getSizeOfType(io_type),
                                       getSizeOfType(share_type), vlen);
  }
};
...
struct TagDebugMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block =
        gr::blocks::tag_debug::make(getSizeOfType(type) * vlen, name, filter);
...
struct AggregationMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto fb_user_taps =
        info.eval_param_vector<double>("fb_user_taps", variables);
    auto fw_user_taps =
        info.eval_param_vector<double>("fw_user_taps", variables);
    auto samp_rate = info.eval_param_value<double>("samp_rate", variables);

    auto block = gr::digitizers::block_aggregation::make(
        alg_id, decim, delay, fir_taps, low_freq, up_freq, tr_width,
        fb_user_taps, fw_user_taps, samp_rate);
...
struct AmplitudeAndPhaseMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block = gr::digitizers::block_amplitude_and_phase::make(
        samp_rate, delay, decim, gain, cutoff, tr_width, hil_win);
...
struct FrequencyEstimatorMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto samp_rate = info.eval_param_value<double>("samp_rate", variables);
    auto sig_window_size =
        info.eval_param_value<int>("sig_window_size", variables);
    auto freq_window_size =
        info.eval_param_value<int>("freq_window_size", variables);
    auto decim = info.eval_param_value<int>("decim", variables);

    auto block = gr::digitizers::freq_estimator::make(
        samp_rate, sig_window_size, freq_window_size, decim);
...
struct SpectralPeaksMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block = gr::digitizers::block_spectral_peaks::make(
        samp_rate, fft_win, med_n, avg_n, prox_n);
...
struct CascadeSinkMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto streaming_sinks_enabled =
        info.param_value<bool>("streaming_sinks_enabled");
    auto triggered_sinks_enabled =
        info.param_value<bool>("triggered_sinks_enabled");
    auto frequency_sinks_enabled =
        info.param_value<bool>("frequency_sinks_enabled");
    auto postmortem_sinks_enabled =
        info.param_value<bool>("postmortem_sinks_enabled");
    auto interlocks_enabled = info.param_value<bool>("interlocks_enabled");
    int pre_samples =
        info.eval_param_value<int>("pre_trigger_samples_raw", variables);
    int post_samples =
        info.eval_param_value<int>("post_trigger_samples_raw", variables);

    return gr::digitizers::cascade_sink::make(
        alg_id, delay, fir_taps, low_freq, up_freq, tr_width, fb_user_taps,
        fw_user_taps, samp_rate, pm_buffer, signal_name, signal_unit,
        streaming_sinks_enabled, triggered_sinks_enabled,
        frequency_sinks_enabled, postmortem_sinks_enabled, interlocks_enabled,
...
struct ChiSquareFitMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block = gr::digitizers::chi_square_fit::make(
        num_samps, function, fun_u, fun_l, num_params, par_names, param_init,
        param_err, param_fit, par_sp_u, par_sp_l, chi_sq);
...
struct DecimateAndAdjustTimebaseMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block = gr::digitizers::decimate_and_adjust_timebase::make(
        decimation, delay, samp_rate);
...
struct EdgeTriggerMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto initial_satate =
        info.eval_param_value<float>("initial_state", variables);
    auto send_udp = info.param_value<bool>("send_udp");
    auto host_list = info.param_value("host_list");

    auto block = gr::digitizers::edge_trigger_ff::make(
        sampling, lo, hi, initial_satate, send_udp, host_list, timeout);
    return block;
  }
};
...
struct ExtractorMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto pre_trigger_window =
        info.eval_param_value<unsigned>("pre_trigger_window", variables);
    auto post_trigger_window =
        info.eval_param_value<unsigned>("post_trigger_window", variables);

    auto block =
        gr::digitizers::demux_ff::make(post_trigger_window, pre_trigger_window);
...
struct FreqSinkMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto acquisition_type =
        info.eval_param_value<int>("acquisition_type", variables);
    auto signal_name = info.param_value("signal_name");
    auto samp_rate = info.eval_param_value<float>("samp_rate", variables);
    ...
    auto block = gr::digitizers::freq_sink_f::make(
        signal_name, samp_rate, nbins, nmeasurements, nbuffers,
        gr::digitizers::freq_sink_mode_t(acquisition_type));
...
struct InterlockGenerationMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block =
        gr::digitizers::interlock_generation_ff::make(max_min, max_max);
...
struct Ps3000aMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto downsampling_factor =
        info.eval_param_value<int>("downsampling_factor", variables);
    ...
    if (enable_ai_a) {
      ...
      auto coupling_ai_a = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_a"));
    ...
    if (enable_ai_b) {
      ...
      auto coupling_ai_b = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_b"));
    ...
    if (enable_ai_c) {
      ...
      auto coupling_ai_c = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_c"));
    ...
    if (enable_ai_d) {
      ...
      auto coupling_ai_d = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_d"));
    ...
    if (trigger_source != "None") {
      if (trigger_source == "Digital") {
        ...
        ps->set_di_trigger(pin_number,
                           static_cast<gr::digitizers::trigger_direction_t>(
                               trigger_direction));
      } else {
        auto trigger_direction = info.param_value<int>("trigger_direction");
    ...
    if (acquisition_mode == "Streaming") {
      ...
      std::ostringstream message;
      message << "Exception in " << __FILE__ << ":" << __LINE__
              << ": unknown acquisition_mode: " << acquisition_mode;
      throw std::invalid_argument(message.str());
    }
    return ps;
  }
...
struct Ps4000aMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    if (enable_ai_e) {
      ...
      auto coupling_ai_e = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_e"));
    ...
    if (enable_ai_f) {
      ...
      auto coupling_ai_f = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_f"));
    ...
    if (enable_ai_g) {
      ...
      auto coupling_ai_g = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_g"));
    ...
    if (enable_ai_h) {
      ...
      auto coupling_ai_h = static_cast<gr::digitizers::coupling_t>(
          info.param_value<int>("coupling_ai_h"));
...
struct Ps6000Maker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    if (enable_ai_a) {
      auto range_ai_a = info.param_value<double>("range_ai_a");
      gr::digitizers::coupling_t coupling_ai_a =
          static_cast<gr::digitizers::coupling_t>(
              info.param_value<int>("coupling_ai_a"));
      auto offset_ai_a = info.param_value<double>("offset_ai_a");
      ps->set_aichan("A", enable_ai_a, range_ai_a, coupling_ai_a, offset_ai_a);
    }
    ...
    if (enable_ai_b) {
      auto range_ai_b = info.param_value<double>("range_ai_b");
      gr::digitizers::coupling_t coupling_ai_b =
          static_cast<gr::digitizers::coupling_t>(
              info.param_value<int>("coupling_ai_b"));
      auto offset_ai_b = info.param_value<double>("offset_ai_b");
      ps->set_aichan("B", enable_ai_b, range_ai_b, coupling_ai_b, offset_ai_b);
    ...
    if (enable_ai_c) {
      auto range_ai_c = info.param_value<double>("range_ai_c");
      gr::digitizers::coupling_t coupling_ai_c =
          static_cast<gr::digitizers::coupling_t>(
              info.param_value<int>("coupling_ai_c"));
      auto offset_ai_c = info.param_value<double>("offset_ai_c");
      ps->set_aichan("C", enable_ai_c, range_ai_c, coupling_ai_c, offset_ai_c);
    ...
    if (enable_ai_d) {
      auto range_ai_d = info.param_value<double>("range_ai_d");
      gr::digitizers::coupling_t coupling_ai_d =
          static_cast<gr::digitizers::coupling_t>(
              info.param_value<int>("coupling_ai_d"));
      auto offset_ai_d = info.param_value<double>("offset_ai_d");
      ps->set_aichan("D", enable_ai_d, range_ai_d, coupling_ai_d, offset_ai_d);
...
struct PostMortemSinkMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto sink = gr::digitizers::post_mortem_sink::make(signal_name, signal_unit,
                                                       samp_rate, buffer_size);
    return sink;
  }
};
...
struct SignalAveragerMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block =
        gr::digitizers::signal_averager::make(n_ports, window_size, samp_rate);
...
struct StftAlgorithmsMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto alg_id = static_cast<gr::digitizers::stft_algorithm_id_t>(
        info.eval_param_value<int>(
            "alg_id", variables)); // static_cast, since eval_param_value cannot
                                   // handle enums :(
    ...
    auto block = gr::digitizers::stft_algorithms::make(
        samp_rate, delta_t, win_size, win_type, alg_id, fq_low, fq_hi, nbins);
...
struct StftGoertzlDynamicMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto block = gr::digitizers::stft_goertzl_dynamic_decimated::make(
        samp_rate, delta_t, win_size, nbins, bound_decim);
...
struct TimeDomainSinkMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    auto signal_unit = info.param_value("signal_unit");
    auto samp_rate = info.eval_param_value<double>("samp_rate", variables);
    auto output_package_size =
        info.eval_param_value<size_t>("output_package_size", variables);
    auto pre_samples = info.eval_param_value<int>("pre_samples", variables);
    auto post_samples = info.eval_param_value<int>("post_samples", variables);
    auto mode = info.param_value<int>("acquisition_type");

    if (mode == gr::digitizers::time_sink_mode_t::TIME_SINK_MODE_TRIGGERED)
      return gr::digitizers::time_domain_sink::make(
          signal_name, signal_unit, samp_rate,
          static_cast<gr::digitizers::time_sink_mode_t>(mode), pre_samples,
          post_samples);
    else
      return gr::digitizers::time_domain_sink::make(
          signal_name, signal_unit, samp_rate,
          static_cast<gr::digitizers::time_sink_mode_t>(mode),
...
struct TimeRealignmentMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    float triggerstamp_matching_tolerance = info.eval_param_value<float>(
        "triggerstamp_matching_tolerance", variables);
    float max_buffer_time =
        info.eval_param_value<float>("max_buffer_time", variables);

    auto block = gr::digitizers::time_realignment_ff::make(
        info.id, user_delay, triggerstamp_matching_tolerance, max_buffer_time);
    return block;
  }
};
...
struct AmplitudePhaseAdjusterMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    return gr::digitizers::amplitude_phase_adjuster::make(ampl_cal, phi_usr,
                                                          phi_fq_usr);
  }
};

static std::vector<float>
makeBandPassFilterFloat(const BlockInfo &info,
                        const std::vector<BlockInfo> &variables) {
  assert(info.key == band_pass_filter_taps_key);
  std::string type = info.param_value("type");
  if (type != "taps_real") {
    std::ostringstream message;
    message << "Exception in " << __FILE__ << ":" << __LINE__
            << ": Wrong Filter Type: '" << type
            << "' selected for filter: " << info.key << " .";
    throw std::invalid_argument(message.str());
  }
  ...
  float low_cutoff_freq =
      info.eval_param_value<float>("low_cutoff_freq", variables);
  float high_cutoff_freq =
      info.eval_param_value<float>("high_cutoff_freq", variables);
  ...
  return gr::filter::firdes::band_pass(
      ...
      (gr::filter::firdes::win_type)win_type, beta);
}

static std::vector<gr_complex>
makeBandPassFilterComplex(const BlockInfo &info,
                          const std::vector<BlockInfo> &variables) {
  assert(info.key == band_pass_filter_taps_key);
  std::string type = info.param_value("type");
...
static std::vector<float>
makeFloatFilter(const BlockInfo &info,
                const std::vector<BlockInfo> &variables) {
  ...
  std::ostringstream message;
  message << "Exception in " << __FILE__ << ":" << __LINE__
          << ": So far the type: " << info.key << " is not supported.";
  throw std::invalid_argument(message.str());
}

static std::vector<gr_complex>
makeComplexFilter(const BlockInfo &info,
                  const std::vector<BlockInfo> &variables) {
  if (info.key == band_pass_filter_taps_key)
...
struct FreqXlatingFirFilterMaker : BlockMaker {
  gr::basic_block_sptr make(const BlockInfo &info,
    ...
    double center_freq =
        info.eval_param_value<double>("center_freq", variables);
    double sampling_freq =
        info.eval_param_value<double>("samp_rate", variables);
    ...
    if (!found) {
      std::ostringstream message;
      message << "Exception in " << __FILE__ << ":" << __LINE__
              << ": Filter TAP named '" << taps_name << "' not found.";
      throw std::invalid_argument(message.str());
    }

    if (filter_type_string == "ccc")
      return gr::filter::freq_xlating_fir_filter_ccc::make(
          decim, makeComplexFilter(tap, variables), center_freq, sampling_freq);
    else if (filter_type_string == "ccf")
      return gr::filter::freq_xlating_fir_filter_ccf::make(
          decim, makeFloatFilter(tap, variables), center_freq, sampling_freq);
    else if (filter_type_string == "fcc")
      return gr::filter::freq_xlating_fir_filter_fcc::make(
          decim, makeComplexFilter(tap, variables), center_freq, sampling_freq);
    else if (filter_type_string == "fcf")
      return gr::filter::freq_xlating_fir_filter_fcf::make(
          decim, makeFloatFilter(tap, variables), center_freq, sampling_freq);
    else if (filter_type_string == "scc")
      return gr::filter::freq_xlating_fir_filter_scc::make(
          decim, makeComplexFilter(tap, variables), center_freq, sampling_freq);
    else if (filter_type_string == "scf")
      return gr::filter::freq_xlating_fir_filter_scf::make(
          decim, makeFloatFilter(tap, variables), center_freq, sampling_freq);
    else {
      std::ostringstream message;
      message << "Exception in " << __FILE__ << ":" << __LINE__
              << ": unknown FreqXlatingFirFilter type: " << filter_type_string;
      throw std::invalid_argument(message.str());
    }
  }
};
...
BlockFactory::BlockFactory() {
  handlers_b[blocks_null_sink_key] =
      boost::shared_ptr<BlockMaker>(new NullSinkMaker());
  handlers_b[blocks_null_source_key] =
      boost::shared_ptr<BlockMaker>(new NullSourceMaker());
  handlers_b[blocks_uchar_to_float_key] =
      boost::shared_ptr<BlockMaker>(new UcharToFloatMaker());
  handlers_b[blocks_vector_to_stream_key] =
      boost::shared_ptr<BlockMaker>(new VectorToStreamMaker());
  handlers_b[blocks_stream_to_vector_key] =
      boost::shared_ptr<BlockMaker>(new StreamToVectorMaker());
  handlers_b[blocks_vector_to_streams_key] =
      boost::shared_ptr<BlockMaker>(new VectorToStreamsMaker());
  handlers_b[blocks_complex_to_mag_key] =
      boost::shared_ptr<BlockMaker>(new ComplexToMagMaker());
  handlers_b[blocks_complex_to_magphase_key] =
      boost::shared_ptr<BlockMaker>(new ComplexToMagPhaseMaker());
  handlers_b[analog_sig_source_x_key] =
      boost::shared_ptr<BlockMaker>(new SigSourceMaker());
  handlers_b[blocks_throttle_key] =
      boost::shared_ptr<BlockMaker>(new ThrottleMaker());
  handlers_b[blocks_tag_share_key] =
      boost::shared_ptr<BlockMaker>(new TagShareMaker());
  handlers_b[blocks_tag_debug_key] =
      boost::shared_ptr<BlockMaker>(new TagDebugMaker());
  handlers_b[blocks_complex_to_float_key] =
      boost::shared_ptr<BlockMaker>(new ComplexToFloatMaker());
  handlers_b[blocks_float_to_complex_key] =
      boost::shared_ptr<BlockMaker>(new FloatToComplexMaker());

  handlers_b[block_aggregation_key] =
      boost::shared_ptr<BlockMaker>(new AggregationMaker());
  handlers_b[block_amplitude_and_phase_key] =
      boost::shared_ptr<BlockMaker>(new AmplitudeAndPhaseMaker());
  handlers_b[block_complex_to_mag_deg_key] =
      boost::shared_ptr<BlockMaker>(new ComplexToMagDegMaker());
  handlers_b[block_demux_key] = boost::shared_ptr<BlockMaker>(new DemuxMaker());
  handlers_b[block_scaling_offset_key] =
      boost::shared_ptr<BlockMaker>(new ScalingOffsetMaker());
  handlers_b[block_spectral_peaks_key] =
      boost::shared_ptr<BlockMaker>(new SpectralPeaksMaker());
  handlers_b[freq_estimator_key] =
      boost::shared_ptr<BlockMaker>(new FrequencyEstimatorMaker());
  handlers_b[cascade_sink_key] =
      boost::shared_ptr<BlockMaker>(new CascadeSinkMaker());
  handlers_b[chi_square_fit_key] =
      boost::shared_ptr<BlockMaker>(new ChiSquareFitMaker());
  handlers_b[decimate_and_adjust_timebase_key] =
      boost::shared_ptr<BlockMaker>(new DecimateAndAdjustTimebaseMaker());
  handlers_b[edge_trigger_ff_key] =
      boost::shared_ptr<BlockMaker>(new EdgeTriggerMaker());
  handlers_b[edge_trigger_receiver_f_key] =
      boost::shared_ptr<BlockMaker>(new EdgeTriggerReceiverMaker());
  handlers_b[demux_ff_key] =
      boost::shared_ptr<BlockMaker>(new ExtractorMaker());
  handlers_b[freq_sink_f_key] =
      boost::shared_ptr<BlockMaker>(new FreqSinkMaker());
  handlers_b[function_ff_key] =
      boost::shared_ptr<BlockMaker>(new FunctionMaker());
  handlers_b[interlock_generation_ff_key] =
      boost::shared_ptr<BlockMaker>(new InterlockGenerationMaker());
  handlers_b[picoscope_3000a_key] =
      boost::shared_ptr<BlockMaker>(new Ps3000aMaker());
  handlers_b[picoscope_4000a_key] =
      boost::shared_ptr<BlockMaker>(new Ps4000aMaker());
  handlers_b[picoscope_6000_key] =
      boost::shared_ptr<BlockMaker>(new Ps6000Maker());
  handlers_b[post_mortem_sink_key] =
      boost::shared_ptr<BlockMaker>(new PostMortemSinkMaker());
  handlers_b[signal_averager_key] =
      boost::shared_ptr<BlockMaker>(new SignalAveragerMaker());
  handlers_b[stft_algorithms_key] =
      boost::shared_ptr<BlockMaker>(new StftAlgorithmsMaker());
  handlers_b[stft_goertzl_dynamic_key] =
      boost::shared_ptr<BlockMaker>(new StftGoertzlDynamicMaker());
  handlers_b[time_domain_sink_key] =
      boost::shared_ptr<BlockMaker>(new TimeDomainSinkMaker());
  handlers_b[time_realignment_key] =
      boost::shared_ptr<BlockMaker>(new TimeRealignmentMaker());
  handlers_b[wr_receiver_f_key] =
      boost::shared_ptr<BlockMaker>(new WrReceiverMaker());
  handlers_b[amplitude_phase_adjuster_key] =
      boost::shared_ptr<BlockMaker>(new AmplitudePhaseAdjusterMaker());
  handlers_b[freq_xlating_fir_filter_xxx_key] =
      boost::shared_ptr<BlockMaker>(new FreqXlatingFirFilterMaker());
}
...
void BlockFactory::common_settings(gr::basic_block_sptr block,
  ...
  if (info.is_param_set("minoutbuf")) {
    ...
    if (minoutbuf > 0) {
      ...
      gr::hier_block2_sptr hb2_ptr =
          boost::dynamic_pointer_cast<gr::hier_block2>(block);
      if (blk_ptr) {
...
gr::basic_block_sptr
BlockFactory::make_block(const BlockInfo &info,
                         const std::vector<BlockInfo> &variables) {
  auto it = handlers_b.find(info.key);

  if (it == handlers_b.end()) {
    std::ostringstream message;
    message << "Exception in " << __FILE__ << ":" << __LINE__ << ": block type "
            << info.key << " not supported.";
    throw std::invalid_argument(message.str());
  }
...
std::unique_ptr<FlowGraph> make_flowgraph(std::istream &input) {
  ...
  for (const auto info : parser.connections()) {
    ...
    if (std::count(disabled_blocks.begin(), disabled_blocks.end(),
                   info.src_id) ||
        std::count(disabled_blocks.begin(), disabled_blocks.end(),
                   info.dst_id)) {
      continue;
    } else {
      graph->connect(info.src_id, info.src_key, info.dst_id, info.dst_key);
    }
  }
